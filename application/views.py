...
from .models import StudentCourse, PerformanceGrade, Parent, Content, Course, HostelS, Student, ClassInfo, Hostel,edu_upload, \
    TeacherCourse, \
...
from application.forms import StudentForm, ParentSignUpForm, ClassComposeForm, ContentForm, PerformanceGradeForm, \
    ...
    PutFinalGradeForm, BehaviorForm, AdminofficerconstraintForm, HostelCreateForm, HostelAllotForm
...
#         try:
#             hostel = Hostel.objects.get(username=request.POST['Hostel_name'], )
#             return render(request, adm_off,
#                           {'error_message': 'Hostel already Exist.\nTry another Hostel.', 'form': StudentForm()})
#         except Hostel.DoesNotExist:
#             if form.is_valid():
#                 hostel = form.save()
#                 hostel.refresh_from_db()  # load the profile instance created by the signal
#                 hostel = Hostel.objects.create(Hostel_name=hostel, NO_of_rooms=form['NO_of_rooms'].value())
#             messages.success(request, 'Student has been successfully added')
#             return render(request, adm_off, {'form': StudentForm()})
#             # if a GET (or any other method) we'll create a blank form
#     else:
...
@login_required(login_url='application:login')
def hostel_allot(request):
    if request.method == 'POST':
        form = HostelAllotForm(request.POST)
        # try:
        #     user = HostelStudent.objects.get(=request.POST['username'], )
        #     return render(request, 'administrativeOfficer/parentSignup.html',
        #                   {'error_message': 'Username Exist...Try Something Else !', 'form': ParentSignUpForm()})
        # except User.DoesNotExist:
        ...
        if form.is_valid():
            # user = form.save()
            # user.refresh_from_db()  # load the profile instance created by the signal
            # hostel = Hostel.objects.create(user)

            print("################")
            # for student in form.cleaned_data.get('student_id'):
            HostelS.objects.create(student_name=form['student_name'].value(), hostel_name=form['hostel_name'].value(),
                                   room_no=form['room_no'].value())

            messages.success(request, 'Hostel alloted to student successfully')
            ...
            return render(request, 'administrativeOfficer/hostelallot.html', {'form': HostelAllotForm()})
        else:
            return render(request, 'administrativeOfficer/hostelallot.html',
                          {'error_message': 'Invalid Information', 'form': HostelAllotForm()})
    else:
        form = HostelAllotForm()
    return render(request, 'administrativeOfficer/hostelallot.html', {'form': form})
...
def hostel_add(request):
    if request.method == 'POST':
        ...
        # try:
        #     hostel = Hostel.objects.get(Hostel_name=request.POST['Hostel_name'] )
        #     return render(request, 'administrativeOfficer/parentSignup.html',
        #                   {'error_message': 'Hostel Exist...Try Something Else !', 'form': ParentSignUpForm()})
        # except User.DoesNotExist:
        if form.is_valid():
            ...
            # user = form.save()
            # user.refresh_from_db()  # load the profile instance created by the signal
            hostel = Hostel.objects.create(Hostel_name=form.cleaned_data.get('Hostel_name'),
                                           NO_of_rooms=form['No_of_rooms'].value())
            # HostelStudent.objects.create(student_id=student,HostelID=hostel)
            # hostel.NO_of_rooms=250
            messages.success(request, 'Hostel has been successfully added')
            ...
            return render(request, 'administrativeOfficer/parentSignup.html', {'form': HostelCreateForm()})
        else:
            # print("******************")
            return render(request, 'administrativeOfficer/parentSignup.html',
                          {'error_message': 'Invalid Information', 'form': HostelCreateForm()})
    else:
...
def educator_login_view(request, educator_id):
    # my_dict = {'allStudentCourses': Course.objects.filter(studentcourse__student_id=student_id)}  # GET STUDENT ID HERE
    # my_dict.update({'parentStudent': ParentStudent.objects.filter(student_id=student_id)})
    # my_dict.update({'student_id': student_id})
    # if ClassInfo.objects.filter(student__ID=student_id).exists():
    ...
    return render(request, 'student/educator_select.html', my_dict,{'educator_id':educator_id})
def student_login_view(request, student_id):
    ...
    if ClassInfo.objects.filter(student__ID=student_id).exists():
        ...
        try:
            ...
            pass
    # return render(request, 'student/after_login.html', my_dict)
    return render(request, 'student/student_select.html', my_dict,{'student_id':student_id})
...
def student_subject(request, student_id, subject):
    print(student_id)
    print(subject)

    return render(request, 'student/subject.html')
def subject_videos(request, student_id, subject):
    print("################################################################################")
    print(student_id)
    print(subject)
    # print(Videos)
    list_of_videos =edu_upload.objects.filter(typex="video",subject=subject)
    #videos_dict = dict()
    context={"videos":list_of_videos}
    #videos_dict"videos"list_of_videos)
    print("YOU ARE HERE")
    print(context)
    return render(request, 'student/Videos')



def subject_contacts(request, student_id, subject,contacts):
    print("################################################################################")
    print(student_id)
    print(subject)
    print(contacts)
    list_of_videos =edu_upload.objects.filter(subject=subject)
    #videos_dict = dict()
    context={"videos":list_of_videos}
    #videos_dict"videos"list_of_videos)
    return render(request, 'student/Videos')
def subject_assignment(request, student_id, subject,assignments):
    print("################################################################################")
    print(student_id)
    print(subject)
    print(assignments)
    list_of_videos =edu_upload.objects.filter(typex="assignment",subject=subject)
    #videos_dict = dict()
    context={"videos":list_of_videos}
    #videos_dict"videos"list_of_videos)
    return render(request, 'student/Videos',context)
def subject_studymaterial(request, student_id, subject,study_material):
    print("################################################################################")
    print(student_id)
    print(subject)
    print(study_material)
    list_of_videos =edu_upload.objects.filter(typex="pdfimg",subject=subject)
    #videos_dict = dict()
    context={"videos":list_of_videos}
    #videos_dict"videos"list_of_videos)
    return render(request, 'student/videos',context)
...
def educator_upload(request):
    ...
    #     return context
    print(request)
    # GetTeacherMasterData(generic.ListView):
    # template_name = 'administrativeOfficer/teacherMasterData.html'
    # context_object_name = 'allTeachers'
    # print(GetTeacherMasterData.get_queryset())
    # def get_queryset(self):
    print("USER")
    print(request.user)
    list_of_teachers = Teacher.objects.filter(user=request.user)
    print("list")
    print(list_of_teachers)
   # print(type(list_of_teachers))
    f_name=""
    last_name=""
    for x in list_of_teachers:
        f_name=x.first_name
        last_name=x.last_name
        print(x.first_name)
        print(x.last_name)
#subject option topic comment
    if request.method == "POST":
        print("HERE")
        educator_full_name=f_name+" "+last_name
        uploaded_file = request.FILES['document']
        soption = request.POST.get('subject')
        ...
        s=fs.save(uploaded_file.name, uploaded_file)
        print("##########################")
        ...
        # print(topic)
        # comment="hjhfhv"
        edu_upload.objects.create(comment=comment,uploaded_on=date_today, uploaded_at=time_today,educator=educator_full_name, typex=typex,subject=subject,item=filename,topic=topic)

        print(s)
    return render(request,'teacher/upload.html')

def educator_myuploads(request):
    my_uploads_assignment=[]
    my_uploads_videos=[]
    my_uploads_sm=[]
    list_of_teachers = Teacher.objects.filter(user=request.user)
    print("list")
    ...
    for x in list_of_teachers:
        ...
        print(x.last_name)
    educator_full_name=f_name+" "+last_name
    for x in edu_upload.objects.filter(typex="assignment",educator=educator_full_name):
        print(x.educator)
        my_uploads_assignment.append(x)
    for x in edu_upload.objects.filter(typex="video",educator=educator_full_name):
        my_uploads_videos.append(x)
    for x in edu_upload.objects.filter(typex="pdfimg",educator=educator_full_name):
        my_uploads_sm.append(x)
    context={'my_uploads_assignment': my_uploads_assignment,'my_uploads_videos': my_uploads_videos,'my_uploads_sm': my_uploads_sm}
    print("int eduuploads")
    print(my_uploads_sm)
    # print(my_uploads)
    # for x in my_uploads:
    #     print(x.subject)
    print("Videos")
    print(my_uploads_videos)
    return render(request,'teacher/my_upload.html',context)


def subject_selected(request, student_id, subject):
    print(student_id)
    print(subject)
    context={"subject":subject, "student_id":student_id}

    return render(request, 'student/base2.html',context)
def student_display(request,student_id,subject,type):

    tp=0
    print("IN STUDENT DISPLAY")
    print(student_id)
    print(subject)
    print(type)
    l=[]
    for x in edu_upload.objects.all():
        print("IN LOOP")
        print(x.typex)
        print(x.subject)
        print(x.topic)
    l = edu_upload.objects.filter(typex=type, subject=subject)
    print("Printing list")
    for x in l:
        print(x.typex)
        print(x.subject)
