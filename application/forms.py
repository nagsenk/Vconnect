...
from .models import Student, ClassInfo, Content, PerformanceGrade, StudentCourse,Hostel,HostelS, Attendance, \
...
class HostelCreateForm(ModelForm):
    No_of_rooms=forms.IntegerField()
    Hostel_name=forms.CharField(required=True)

    class Meta:
        model = Hostel
        fields = ('Hostel_name','No_of_rooms')

    def save(self, commit=True):
        user = super(HostelCreateForm, self).save(commit=False)
      
        if commit:
            user.save()
        return user
class HostelAllotForm(ModelForm):
    # No_of_rooms=forms.IntegerField()
    # Hostel_name=forms.CharField(required=True)
    hostel_name=forms.CharField(required=True)
    ...
    class Meta:
        model = HostelS
        fields = ('room_no','student_name','hostel_name')

    def save(self, commit=True):
        user = super(HostelAllotForm, self).save(commit=False)
